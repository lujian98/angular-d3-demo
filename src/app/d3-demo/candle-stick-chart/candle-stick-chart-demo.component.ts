...
export class AppCandleStickChartDemoComponent implements OnInit {
  ...
  // var date = new Date(elodata.x); (new Date(new Date().getTime() - (20000 * 86400000) + (d * 86400000))),

  private formatData(data) {
    data[0].values.forEach(d => {
      const dt = new Date(new Date().getTime() - (20000 * 86400000) + (d.date * 86400000));
      d.date = new Date(new Date().getTime() - (20000 * 86400000) + (d.date * 86400000));
    });
  }
}
